...
cat <<EOF >/etc/xray/username

---------------------------------------
     Terimakasih sudah menggunakan 
           Script Credit by
-                                     -
		 TomattoVPN Tunneling
---------------------------------------
...
clear
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/rwrtx/vvipsc/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/rwrtx/vvipsc/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
...
Exp1=$(curl https://raw.githubusercontent.com/rwrtx/vvipsc/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
restart_system() {
USRSC=$(wget -qO- https://raw.githubusercontent.com/rwrtx/vvipsc/main/ip | grep $ipsaya | awk '{print $2}')
EXPSC=$(wget -qO- https://raw.githubusercontent.com/rwrtx/vvipsc/main/ip | grep $ipsaya | awk '{print $3}')
TIMEZONE=$(printf '%(%H:%M:%S)T')
TEXT="
<code>🧿───────────────────🧿</code>
...
<code>🧿───────────────────🧿</code>
<i>Automatic Notification from Script</i>
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"https://t.me/https://t.me/TomattoVPN"},{"text":"JOIN","url":"https://t.me/TOMATTO_VPN"}]]}'
...
wget https://raw.githubusercontent.com/rwrtx/sctomatto/main/Fls/limit.sh && chmod +x limit.sh && ./limit.sh
cd
...
wget -O /usr/bin/ws "https://raw.githubusercontent.com/rwrtx/sctomatto/main/Fls/ws" >/dev/null 2>&1
wget -O /usr/bin/tun.conf "https://raw.githubusercontent.com/rwrtx/sctomatto/main/Cfg/tun.conf" >/dev/null 2>&1
wget -O /etc/systemd/system/ws.service "https://raw.githubusercontent.com/rwrtx/sctomatto/main/Fls/ws.service" >/dev/null 2>&1
...
wget https://raw.githubusercontent.com/rwrtx/sctomatto/main/menu/menu.zip
